...
static const char* resolve_font_path(const char* path) {
...
void imgui_init(tenv* env) {
  ...
  for (int i = 0; i < NUM_FONT_SIZES; i++) {
    ...
    usr->imgui_data.mono_font[i] = ImFontAtlas_AddFontFromFileTTF(
        io->Fonts, resolve_font_path("app/res/fonts/mono_regular.ttf"), font_sz, NULL, NULL);

    ImFontAtlas_AddFontFromFileTTF(
        io->Fonts, resolve_font_path("app/res/fonts/iconfont.ttf"), font_sz, &icons_config,
        (const ImWchar[]){0xe900, 0xeaea, 0});

    usr->imgui_data.regular_font[i] = ImFontAtlas_AddFontFromFileTTF(
        io->Fonts, resolve_font_path("app/res/fonts/regular_regular.ttf"), font_sz, NULL, NULL);
    ...
    usr->imgui_data.mono_font_bold[i] = ImFontAtlas_AddFontFromFileTTF(
        io->Fonts, resolve_font_path("app/res/fonts/mono_bold.ttf"), font_sz, NULL, NULL);

    ImFontAtlas_AddFontFromFileTTF(io->Fonts, resolve_font_path("app/res/fonts/iconfont.ttf"),
                                   font_sz, &icons_config,
                                   (const ImWchar[]){0xe900, 0xeaea, 0});

    usr->imgui_data.regular_font_bold[i] = ImFontAtlas_AddFontFromFileTTF(
        io->Fonts, resolve_font_path("app/res/fonts/regular_bold.ttf"), font_sz, NULL, NULL);
